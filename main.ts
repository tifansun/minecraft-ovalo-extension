...
namespace figuras {
    ...
    //% block="relleno con bloque $bloque radio $radio largo $largo posY1 $posY1 posY2 $posY2"
    //% bloque.shadow=minecraftBlock
    //% radio.defl=30
    //% largo.defl=70
    //% posY1.defl=0
    //% posY2.defl=3
    export function relleno(bloque: number, radio: number, largo: number, posY1: number, posY2: number) {
        let x = 0
        let z = 0
        let z2 = 0
        let ubicacion = player.position()
